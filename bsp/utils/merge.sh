...
#   1) current merging mechanism based on git_merge_theirs() doesn't merge properly merge all commits
#      so data shown in 1.0.x Release Notes may be incomplete
#   2) version files in meta-exorint and meta-branding-exorint are overwritten without user interaction 
#      (requires reset to merge changes from origin)
#   3) automatic repo cloning
#   4) merge also other brands automatically (e.g. pgd_ca-1.x.x)
...
#   5) SVN merge handling (e.g. if jmuconfig-app ever has stable branch)
#        OR move hmibrowser to GIT
#   6) if new stable branches are added, recipe patching logic needs to be extended;
#     also need to handle different platforms (bootloader, kernel)
#   7) preserve and use SRCBRANCH to make sure tag really exists on branch

MERGE_FROM_VER="1.999.195"
...
#    fi
    ...
    if [ -z "${TICKET_ID}" ]; then
        ...
        [[ -e .git/refs/remotes/${merge_from} || -e .git/refs/tags/${merge_from} ]] || merge_from="origin/master"
...
merge_toprepos_fixes ()
{
    ...
    case ${repo} in
        meta-exorint|meta-exorint-enterprise|meta-branding-exorint)
            ...
            prompt "Repo '${repo}' requires *SPECIAL CARE*!" \
                "We have preserved original version.inc files upstream changes will be lost (e.g. new platforms)!" \
                "Please make any required changes before proceeding."
            ;;
    esac
}
