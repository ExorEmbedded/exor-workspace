...
#
FEATURE_PACKAGES_qt4-x11-min = "packagegroup-qt-x11-min"
FEATURE_PACKAGES_exorint-appliance-min = "packagegroup-exorint-appliance-min"
...
# FastBoot recipes
#IMAGE_INSTALL += '${@bb.utils.contains("MACHINE_FEATURES", "fastboot", "fastboot-checks", "",d)}'
IMAGE_INSTALL += '${@bb.utils.contains("MACHINE_FEATURES", "fastboot", "fastboot-checks-prechecks", "",d)}'
IMAGE_INSTALL += '${@bb.utils.contains("MACHINE_FEATURES", "fastboot", "fastboot-checks-postchecks", "",d)}'
...
post_process_min() {
    ...
    # set root user hash
    sed 's%^root:[^:]*:%root:${DEFAULT_ROOT_HASH}:%' < ${IMAGE_ROOTFS}/etc/shadow > ${IMAGE_ROOTFS}/etc/shadow.new
    ...
    rm ${IMAGE_ROOTFS}/etc/default/ssh

    # make sure shadow file has its own group (for PAM module access)
    chgrp shadow ${IMAGE_ROOTFS}/etc/shadow
    chmod 440 ${IMAGE_ROOTFS}/etc/shadow
}

ROOTFS_POSTPROCESS_COMMAND += "post_process_min;"
